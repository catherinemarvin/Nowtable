...
Listener.prototype.check = function(req, res, httpUpgrade, head){
  ...
  if (path && path.indexOf('/' + this.options.resource) === 0){
	...
	console.log("Client re-establishing connection.");
	this._onConnection(parts[0], req, res, httpUpgrade, head, parts[1]);
/*        req.connection.end();
...
Listener.prototype._onConnection = function(transport, req, res, httpUpgrade, head, id){
  this.options.log('Initializing client with transport "'+ transport +'"');
  new transports[transport](this, req, res, this.options.transportOptions[transport], head, id);
};
